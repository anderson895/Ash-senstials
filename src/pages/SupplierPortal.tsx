...
import {
  HomeIcon, ArchiveBoxIcon, ChatBubbleLeftRightIcon, ClipboardDocumentListIcon,
  CubeIcon, CurrencyDollarIcon, CheckCircleIcon, ArrowTrendingUpIcon,
...
export default function SupplierPortal({ user, onLogout }: { user: User; onLogout: () => void }): React.ReactElement {
  ...
  const statCards = [
    { label: "Total Revenue",     value: `₱${totalRevenue.toLocaleString()}`, icon: <CurrencyDollarIcon className="w-5 h-5 sm:w-6 sm:h-6" />, color: "from-pink-500 to-rose-500",    bg: "bg-pink-50",   text: "text-pink-600"   },
    { label: "Products Supplied", value: supplyItems.length,                  icon: <CubeIcon           className="w-5 h-5 sm:w-6 sm:h-6" />, color: "from-violet-500 to-purple-500", bg: "bg-violet-50", text: "text-violet-600" },
    { label: "Completed Orders",  value: completedOrders,                     icon: <CheckCircleIcon    className="w-5 h-5 sm:w-6 sm:h-6" />, color: "from-teal-500 to-cyan-500",    bg: "bg-teal-50",   text: "text-teal-600"   },
  ];
        ...
        <main className="flex-1 p-4 sm:p-6 lg:p-8 overflow-y-auto min-w-0">
          ...
          {tab === "dashboard" && (
            <div className="space-y-5 sm:space-y-6">
              <h1 className="flex items-center gap-2 text-xl sm:text-2xl font-bold text-pink-700"
                ...
                <HomeIcon className="w-6 h-6 sm:w-7 sm:h-7 text-pink-500" /> Supplier Dashboard
              </h1>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <Card className="border-pink-100">
                  <CardHeader className="pb-2">
                    <CardTitle className="text-pink-700 text-base flex items-center gap-2"
                      style={{ fontFamily: "'Playfair Display', Georgia, serif" }}>
                  ...
                  <CardContent>
                    <div className="space-y-2 sm:space-y-3">
                      {supplyOrders.map(o => (
                        <div key={o.id} className="flex justify-between items-center py-1.5 sm:py-2 border-b border-pink-50 last:border-0">
                          <div>
                            <p className="font-semibold text-xs sm:text-sm">{o.product}</p>
                            <p className="text-[10px] sm:text-xs text-gray-400">Qty: {o.qty} · {o.date}</p>
                          </div>
                ...
                <Card className="border-pink-100">
                  ...
                  <CardContent className="space-y-2 sm:space-y-3">
                    {supplyItems.map(i => (
                      <div key={i.name} className="flex items-center gap-2 sm:gap-3">
                        <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-lg overflow-hidden bg-pink-50 shrink-0">
                          <img src={i.image} alt={i.name} className="w-full h-full object-cover" />
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="text-[10px] sm:text-xs font-semibold text-gray-700 mb-1">{i.name}</p>
          ...
          {tab === "inventory" && (
            <div className="space-y-5 sm:space-y-6">
              <h1 className="flex items-center gap-2 text-xl sm:text-2xl font-bold text-pink-700"
                style={{ fontFamily: "'Playfair Display', Georgia, serif" }}>
                <ArchiveBoxIcon className="w-6 h-6 sm:w-7 sm:h-7 text-pink-500" /> Inventory to Supply
              </h1>
                  ...
                  <TableBody>
                    {supplyItems.map(i => (
                      <TableRow key={i.name} className="hover:bg-pink-50/30">
                        ...
                        <TableCell className="font-bold text-teal-600 text-xs sm:text-sm whitespace-nowrap">{i.supplied} units</TableCell>
                        <TableCell className={`font-bold text-xs sm:text-sm whitespace-nowrap ${i.needed > 0 ? "text-pink-600" : "text-gray-400"}`}>
          ...
          {tab === "sales" && (
            <div className="space-y-5 sm:space-y-6">
              <h1 className="flex items-center gap-2 text-xl sm:text-2xl font-bold text-pink-700"
                style={{ fontFamily: "'Playfair Display', Georgia, serif" }}>
                <ArrowTrendingUpIcon className="w-6 h-6 sm:w-7 sm:h-7 text-pink-500" /> Sales Report
              </h1>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <Card className="border-pink-100">
                  <CardHeader className="pb-2">
                  ...
                  <CardContent className="space-y-4">
                    {[{ month: "January", rev: 5200 }, { month: "February", rev: 7800 }, { month: "March", rev: 5450 }].map(r => (
                      <div key={r.month}>
                        ...
                        <div className="h-2.5 sm:h-3 bg-pink-100 rounded-full overflow-hidden">
                    ...
                    <div className="mt-2 p-3 bg-pink-50 rounded-xl flex items-center justify-between">
                      ...
                      <span className="text-base sm:text-lg font-black text-pink-600">₱18,450</span>
                    </div>
                  </CardContent>
                </Card>
                <Card className="border-pink-100">
                  ...
                  <CardContent>
                    <div className="space-y-2 sm:space-y-3">
                      {supplyOrders.map(o => (
                        <div key={o.id} className="flex justify-between items-start py-1.5 sm:py-2 border-b border-pink-50 last:border-0">
                          <div>
                            <p className="font-bold text-xs sm:text-sm">{o.id} – {o.product}</p>
                            <p className="text-[10px] sm:text-xs text-gray-400 mt-0.5">Qty: {o.qty} · {o.date}</p>
                          </div>
                          <StatusBadge status={o.status} />
                        </div>
                      ))}
                    ...
                    <CardContent className="py-3 sm:py-4 flex items-center gap-3 sm:gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-center mb-0.5 sm:mb-1">
                          <span className="font-bold text-gray-800 text-xs sm:text-sm truncate">{m.from}</span>
